...
draft = false
title = "Finding vector perpendicural to another vector"
tags = ['3D', 'math']
+++

Hello, in this post I will present solution for math problem I stumbled upon
recently. The task was given as folows:

> Given arbitrary 3D vector from 3D space find any vector that is perpendicural to
> it. Note that there is infinite numer of vectors perpendicural to given one.
...
## Angle beetween vectors
Using dot product we can compute angle beetween vectors. Let's write formula
...
If vectors have to be perpendicural to each other the angle beetween them needs to be equal
@@90^\circ@@ (@@\frac{\pi}{4}@@).

We know that:
$$\cos(\frac{\pi}{4}) = 0$$

So lets insert it into out previos equation:
...
then angle beetween them is equal to @@90^\circ@@ (@@\frac{\pi}{4}@@).

## 2D example
Let's suppose we have vector @@\vec{A} = (A_x, A_y)@@ and we want to find vector that is
perpendicural to it. Starting with formula:
$$\vec{A} \cdot \vec{U} = 0$$
...
of perpendicural vectors that we can find will be correct solution - let's use some assumption. If we
arbitrarly set @@\vec{U}_x = 1@@ then our equation will turn into:
...
The you need to set @@\vec{U}_y = 1@@ and follow previos steps which will lead us to:
$$\vec{U} = \left(-\left(\frac{\vec{A}_y}{\vec{A}_x}\right), 1\right)$$

If bouth @@\vec{A}_x@@ and @@\vec{A}_y@@ are equal to @@0@@ then we can't find perpendicural vector
beacause @@\vec{A}@@ has a length of @@0@@.

## 3D example
After showing 2D example 3D one will be more understandable. Now lets suppose we have vector
@@\vec{A} = (A_x, A_y, A_z)@@ and we want to find vector that is perpendicural to it.
...
another component of @@\vec{U}@@ and set it arbitrarly to @@1@@.

## Final python example code
To sum up I prepared simple Python method that will compute vector perpendicural to another for 3D
